...
REGISTRAR owns its own declarative ``Base``. When mounted in a host (lotek), ``register()`` is handed the
host engine/session factory and simply creates REGISTRAR's tables in the shared database; the tables are
``registrar_``-prefixed so they never collide with host tables.
...
def create_all(engine) -> None:
    """Create REGISTRAR's tables, then additively add any new columns (and their indexes) to pre-existing
    ...
    from registrar import models  # noqa: F401  (import for side effect: register the mapped tables)
