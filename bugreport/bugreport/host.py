...
Mirror of scribble/host.py. Reads ``bugreport.deps.get_config().extras`` at call time and FAILS CLOSED
(503) when the host injected nothing — standalone Bugreport has no PAT scheme, so a machine route with no
host gate must refuse, never run unauthenticated. Only ``bugreport/api_pat.py`` uses this.
"""
...
from bugreport.deps import get_config
