...
`preview_client` fixture below) rather than `scribble.api.api_bp`/`scribble.blueprint.bp`: those module-
...
from scribble.templating_api import register as register_templating_api
...
def test_builtin_key_not_silently_overwritten_by_custom_value(session_factory):
    ...
    (builtin=False) rows are overlaid automatically. See scribble/templating/context.py precedence."""
    with session_factory() as db:
...
def test_preview_endpoint_doc_mode(preview_client, preview_session_factory):
    ...
    resp = preview_client.post("/scribble/api/preview", json={"engagement_id": eng_id, "doc": doc})
    assert resp.status_code == 200
    data = resp.get_json()
...
def test_preview_endpoint_finding_mode(preview_client, preview_session_factory):
    ...
    resp = preview_client.post(
        "/scribble/api/preview", json={"engagement_id": eng_id, "finding_id": finding_id}
    )
    assert resp.status_code == 200
    data = resp.get_json()
...
def test_preview_endpoint_requires_engagement_id(preview_client):
    resp = preview_client.post("/scribble/api/preview", json={"text": "hi"})
...
def test_preview_endpoint_requires_some_input(preview_client, preview_session_factory):
    ...
    resp = preview_client.post("/scribble/api/preview", json={"engagement_id": eng_id})
    assert resp.status_code == 400
...
def test_builtin_keys_importable_from_package_root():
    ...
    # import from `scribble.templating` -- confirm WS6's __init__.py edits didn't shadow/break it.
