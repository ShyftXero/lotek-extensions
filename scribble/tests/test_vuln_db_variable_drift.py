...
(``scribble/seed/lotek_vulnerabilities.json``) must be a variable the pipeline can actually populate --
either a structural ``BUILTIN_KEYS`` name or a DB-declared ``TemplateVariable`` (``scribble/seed/
...
from scribble.templating import known_variable_keys, lint_doc

_LOTEK_SEED = Path(__file__).parent.parent / "scribble" / "seed" / "lotek_vulnerabilities.json"
...
def test_every_lotek_template_token_is_known_to_the_pipeline(session_factory):
    ...
    assert unknown_by_template == {}, (
        ...
        "either declare the fact -> variable mapping in scribble/seed/report_variables.json, or fix the "
