...
Scribble-owned ids on the machine surface are UUIDv7, so their routes use Flask's built-in `<uuid:...>`
converter where a route takes only Scribble ids. The engagement-scoped routes that deliberately accept
**either** the Scribble engagement UUID **or** the stored core engagement id (`core_engagement_id`) use a
plain `<engagement_id>` string and resolve through `api_pat._resolve_engagement`, which collapses malformed,
unknown, and unauthorized ids to the same 404. There is therefore no bare `<int:>` machine-route lookup left
to overflow into `db.get()`.
