...
``known_keys=known_variable_keys(session)`` (see ``scribble.templating.context``) to also recognize
...
from scribble.templating.context import build_full_context
from scribble.templating.resolver import BUILTIN_KEYS, make_var_resolver, resolve_doc
...
def resolve_finding(session, finding, *, artifact_url=None) -> dict[str, str]:
    ...
    Combines :func:`scribble.templating.context.build_full_context` (built-ins + engagement/finding
