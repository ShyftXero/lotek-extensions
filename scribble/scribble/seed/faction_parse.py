...
(``scribble/seed/loader.py``) wrapped that whole blob as literal text via ``schema.doc_from_text``, so
reports showed literal ``# Description <p>...</p>`` instead of real paragraphs/lists. This module turns
that source format into real ``scribble.content.schema`` ProseMirror doc JSON.
...
- Token normalization (``normalize_tokens``): FACTION's ``{{client}}`` / bare ``CLIENT`` both become
  Scribble's real ``{{COMPANY_NAME}}`` builtin. FACTION's ``{{.foo}}`` syntax (e.g. ``{{.pass_pol}}``)
...
def normalize_tokens(text: str) -> str:
    """FACTION -> Scribble token normalization. See module docstring for the rationale."""
    if not text:
        return text
...
def bold_lead_paragraph(text: str) -> dict[str, Any]:
    ...
    inside the ``description`` block (Scribble has no separate impact block)."""
    return {
...
# ---------------------------------------------------------------------------
# FACTION record -> Scribble content blocks
# ---------------------------------------------------------------------------


def build_template_blocks(description: str | None, recommendation: str | None) -> dict[str, dict[str, Any]]:
    """Map one FACTION record's ``Description``/``Recommendation`` fields onto Scribble's
    ...
    - ``description`` <- the "# Description" section, with the "# Impact" section (if present and
      non-empty) folded in underneath a bold "Impact" lead paragraph, since Scribble has no separate
      impact block.
