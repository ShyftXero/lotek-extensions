...
See ``scribble/seed/faction_parse.py`` for the parser and the FACTION->Scribble token normalization
...
def seed_vuln_map(session, json_path: str | Path | None = None) -> int:
    ...
    ``json_path`` defaults to ``scribble/seed/lotek_vuln_map.json`` (the 11-entry set transcribed
    ...
    for spec in specs:
        ...
        existing = session.scalar(
            select(ScribbleVulnMap).where(
                ScribbleVulnMap.source.is_(source) if source is None else ScribbleVulnMap.source == source,
                ScribbleVulnMap.title_pattern.is_(title_pattern)
                if title_pattern is None
                else ScribbleVulnMap.title_pattern == title_pattern,
                ScribbleVulnMap.dedupe_prefix.is_(dedupe_prefix)
                if dedupe_prefix is None
                else ScribbleVulnMap.dedupe_prefix == dedupe_prefix,
            )
        )
        ...
        if not templates:
            raise RuntimeError(
                f"scribble VulnMap seed: no active scribble_vuln_templates row named {name!r} -- "
                "import_vuln_templates may not have run yet, or this vuln-DB entry was renamed/removed "
                "from lotek_vulnerabilities.json"
            )
        if len(templates) > 1:
            logger.warning(
                "scribble VulnMap seed: %d active scribble_vuln_templates rows named %r; "
...
def seed_report_variables(session, json_path: str | Path | None = None) -> int:
    ...
    ``.target_column``, CONTRACT-FACTS §4.1/§4.3) from ``scribble/seed/report_variables.json``.
    ...
    the ``scribble_variables`` row -- never in a per-source Python dispatch table.
...
def import_checklist_templates(session, checklists_dir: str | Path | None = None) -> int:
    ...
    """
    from scribble.checklists import normalize_template_dict
