...
def _render_activity_appendix(ctx: ReportContext) -> str:
    """Optional engagement activity trail (lotek#442) — a timestamped log of report-relevant actions
    (finding added, evidence uploaded, diagram created), OPT-IN via the ``activity_log`` template block
    (the "checkbox"). Renders nothing when empty, so a template that includes the block for an engagement
    with no activity stays byte-identical and its TOC entry is likewise omitted (see ``_toc_entries``).
    Bounded by ``_MAX_APPENDIX_ITEMS`` with a visible note; the heading reports the TRUE total. Every
    field is ``_esc``-escaped — the summaries embed engagement-controlled text (finding titles, filenames).
    """
    ...
    withheld = total - len(shown)
    rows = "".join(
        f'<tr><td class="act-ts">{_esc(e.timestamp)}</td>'
        f'<td class="act-kind">{_esc(e.kind)}</td>'
        f'<td class="act-summary">{_esc(e.summary)}</td></tr>'
        for e in shown
    )
    note = (
        f'<p class="muted">{withheld} earlier entr{"ies are" if withheld != 1 else "y is"} not listed '
        f"here (this appendix lists at most {_MAX_APPENDIX_ITEMS}).</p>"
        if withheld > 0
        else ""
    )
    return (
        '<section class="sec group" id="sec-activity_log">'
        '<h2 class="sec-h">Activity Log <span class="chev">▾</span>'
        f'<span class="count">{total} entr{"ies" if total != 1 else "y"}</span></h2>'
        '<div class="sec-body"><p class="muted">Timestamped record of engagement activity — findings '
        "added, evidence uploaded, and attack-path diagrams created — for the engagement audit trail.</p>"
        '<table class="act-table"><thead><tr><th>Time</th><th>Type</th><th>Activity</th></tr></thead>'
        f"<tbody>{rows}</tbody></table>{note}</div></section>"
    )
...
def _toc_entries(ctx: ReportContext, blocks: tuple[str, ...]) -> list[tuple[int, str, str, str]]:
    ...
    for key in blocks:
        if key == "summary":
            ...
            if ctx.activity_log:
                entries.append((1, "sec-activity_log", "Activity Log", ""))
    return entries
...
def _render_block_by_key(
    ...
    if key == "activity_log":
        return _render_activity_appendix(ctx)
...
/* Activity Log appendix (lotek#442) — a plain timestamped table, same visual language as the
