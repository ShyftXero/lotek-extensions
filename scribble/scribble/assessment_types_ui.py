...
(Internal / External / Web App / Device-Mobile, see `scribble/seed/loader.py::_DEFAULT_TYPES`) but users
...
This module does NOT edit `scribble/__init__.py`; the driver wires `register` into
...
from scribble.models import AssessmentType, FindingGroup
...
def register(api_bp, bp) -> None:
    ...
    def assessment_types():
        with open_session() as db:
            ...
            return render_template("scribble/assessment_types.html", rows=rows)
        ...
        with open_session() as db:
            ...
            return (
                jsonify(
                    ok=True,
                    id=at.id,
                    redirect=url_for("scribble.assessment_types"),
                ),
                201,
            )
