/*!
 * scribble/static/outbox.js — WS14 client resilience: a dependency-free, IndexedDB-backed upload
 ...
 *
 * Contract (consumed by scribble/static/artifacts.js and scribble/static/editor.js):
 *
 *   var tempId = ScribbleOutbox.enqueueUpload({
 *     tempId: "optional-caller-chosen-id",   // generated if omitted; returned either way
 *     url: "/scribble/api/artifacts",         // POST target -- reused as-is, no new server routes
 *     blob: fileOrBlob,
 *     filename: "screenshot.png",
 *     fields: { engagement_id: "1", finding_id: "2", caption: "..." },  // extra multipart fields
 *   });
 *
 *   ScribbleOutbox.on("resolved", function (tempId, serverJson, op) { ... });  // 2xx: op succeeded
 *   ScribbleOutbox.on("failed",   function (tempId, error, op) { ... });      // 4xx: dropped, won't retry
 *   ScribbleOutbox.pendingCount() -> number                                   // outbox + external pending
 *   ScribbleOutbox.isGuardArmed() -> boolean                                  // beforeunload currently armed
 *   ScribbleOutbox.setExternalPending(key, isPending)  // let other subsystems (e.g. editor.js's text
 *                                                       // autosave) share this module's beforeunload guard
 *   ScribbleOutbox.flush() -> Promise<void>            // re-attempt every durably-queued pending op now
 *
...
(function (window) {
  ...
  if (window.ScribbleOutbox) return;

  var DB_NAME = "scribble-outbox";
  var DB_VERSION = 1;
  var STORE = "uploads";
  var LOCK_NAME = "scribble-outbox-flush";

  // Retry/backoff tunables. Defaults are the production values; a page may override them by setting
  // `window.__scribbleOutboxConfig` BEFORE this script loads (used by the e2e suite to exercise the
  // attempt cap without waiting out the full production backoff schedule).
  var _cfg = (window && window.__scribbleOutboxConfig) || {};
    ...
    (_listeners[event] || []).slice().forEach(function (fn) {
      try {
        ...
        if (window.console && window.console.error) {
          window.console.error("[scribble-outbox] listener error", e);
        }
      }
    });
  }
