/*!
 * Scribble live co-editing client (WS11 Phase B) — scribble/static/collab.js
 * =====================================================================================
 *
 * Bridges the existing rich-text editor (`scribble/static/editor.js`) to the server-side CRDT room
 * (`scribble/collab/crdt.py`) over the `/ws/findings/<id>/blocks/<block>` websocket, using a **real,
 * vendored Yjs client** (`scribble/static/lib/yjs.mjs` + `scribble/static/lib/y-protocols/*`, see
 * `scribble/static/lib/VENDOR.md` for exact versions/provenance/verification) — not a stub. The wire
 ...
 *   mapping as the server's `scribble/collab/pm_yjs.py` (re-implemented here in JS — see
 ...
 *   HTTP-polling presence (`scribble/collab/presence.py`) — presence.py keeps working wherever this
 ...
 *   </script>
 *
 * (`window.ScribbleCollab.attach`/`.isSupported` are also exposed as a non-module-script fallback.)
...
// ---------------------------------------------------------------------------- wire message types
// Matches scribble/collab/crdt.py exactly (pycrdt.YMessageType): 0 = sync, 1 = awareness.
var MESSAGE_SYNC = 0;
var MESSAGE_AWARENESS = 1;

// ---------------------------------------------------------------------------- ProseMirror JSON <-> Yjs
// A JS port of scribble/collab/pm_yjs.py's mapping. Kept in lockstep with that module -- see
// scribble/static/lib/VENDOR.md for how the two were verified to agree on an identical fixture.
...
function deriveWsUrl(opts) {
  ...
  var apiBase = opts.apiBase || "/scribble/api";
  ...
  ydoc.on("update", function (update, origin) {
    if (origin === REMOTE_ORIGIN) {
      try {
        ...
        console.error("[scribble-collab] failed to render remote update", e);
      }
      return;
    }
    var encoder = encoding.createEncoder();
