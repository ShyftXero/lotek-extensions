...
def _model() -> dict:
    zones = [
        {"id": "attacker", "title": "ATTACKER INFRA", "subtitle": "Chitauri 47.232.83.0/24", "accent": "red", "order": 0},
    ...
    nodes = [
        {"id": "opfor-1", "label": "opfor-1", "ip": "93.82.71.24", "domain": "juliet.future.org",
         "zone": "attacker", "row": 0, "role": "rshell", "states": []},
        {"id": "opfor-2", "label": "opfor-2", "ip": "97.49.38.28", "domain": "core-align.com",
         "zone": "attacker", "row": 1, "role": "c2", "states": [{"at": 0, "label": "QUANTUMCAT C2"}]},
        {"id": "opfor-3", "label": "opfor-3", "ip": "72.39.92.25", "domain": "guest.network.com",
         "zone": "attacker", "row": 2, "role": "stager",
         "reIp": {"at": 15, "ip": "209.125.93.29", "domain": "coast.titan.org"},
         "states": [{"at": 0, "label": "CATSONBROADWAY"}, {"at": 15, "label": "SSH TUNNEL"}]},
        {"id": "opfor-4", "label": "opfor-4", "ip": "48.29.174.30", "domain": "db.inspire.net",
...
def _phases() -> list[dict]:
    return [
        ...
        {"n": 3, "title": "Stage the beacon (.elf)", "tactics": [{"label": "Command & Control", "kind": "c2"}],
         ...
         "blue": {"tool": "Host / Elastic · Network", "finding": "Beacon .elf staged via CatsonBroadway from opfor-3 (guest.network.com / 72.39.92.25). On range: sftp-server wrote .ntp_key (Remote File Creation in World Writeable Directory).",
        ...
        {"n": 4, "title": "Beacon callback — raw TCP / 443", "tactics": [{"label": "Command & Control", "kind": "c2"}],
         ...
         "blue": {"tool": "Security Onion · Zeek/Suricata · Linux process audit", "finding": "Beacon executes with argv spoof exec -a '[kworker/0:1-events]' and callbacks to C2 core-align.com / 97.49.38.28 over raw TCP/443.",
                  "query": "destination.ip:97.49.38.28 OR dns.query:core-align.com\nauditd/EDR: process.name:*kworker* AND NOT executable:/proc/*",
        ...
        {"n": 12, "title": "Beacon mesh: IT ⇄ OT DMZ", "tactics": [{"label": "Command & Control", "kind": "mesh"}],
         ...
         "blue": {"tool": "SO egress + host — unvalidated", "finding": "Beacon on engineering-2; peer mesh with spark-ot-it-3. Third C2 arc to core-align.com plus amber P2P mesh.",
                  "query": "destination.ip:97.49.38.28 and source.ip:(172.18.5.22 or 172.18.8.33)",
        ...
        {"n": 15, "title": "SSH + SOCKS chain to OT", "tactics": [{"label": "Command & Control", "kind": "tunnel"}],
         ...
         "note": "Kill CatsonBroadway on opfor-3 first. The same box that staged the beacon is repurposed as the SSH tunnel egress — tear down the staging service, then it re-IPs to 209.125.93.29 (coast.titan.org) as the tunnel comes up.",
         "blue": {"tool": "SO egress — unvalidated", "finding": "SSH tunnel from IT Ubuntu out to opfor-3; opfor-3 re-IPs to coast.titan.org / 209.125.93.29; SOCKS terminates on engineering-2; opfor-4 (db.inspire.net) as backup.",
                  "query": "destination.ip:(72.39.92.25 or 209.125.93.29 or 48.29.174.30)\ndns.query:(guest.network.com or coast.titan.org or db.inspire.net)",
