...
Mirror of scribble/host.py (same two constraints: ``register()`` fills ``cfg.extras`` AFTER route wiring,
and wiring is a process singleton while ``extras`` is per-app). Everything reads
``vector.deps.get_config().extras`` at call time and FAILS CLOSED (503) when the host injected nothing —
standalone Vector has no PAT scheme, so a machine route with no host gate must refuse, never run
unauthenticated. Only ``vector/api_pat.py`` (the machine API) uses this.
"""
...
def require_scope(scope: str):
    ...
    the scope on the wrapper so the host's OpenAPI generator can introspect it."""
