...
/*!
 * lotek_kit/static/reporting-editor.js — the SHARED rich-text reporting editor primitive.
 * =======================================================================================
 *
 * Extracted from scribble's original editor into the kit so every extension that authors report text
 * (scribble today; bugreport and others later) mounts ONE canonical editor, updated in one place,
 * rather than copy-pasting it. It is a minimal, dependency-free `contenteditable` editor that
 * reads/writes ProseMirror-JSON, with a TipTap drop-in seam for a future real-TipTap upgrade.
 *
 ...
 * Define `window.LotekReportingEditorTipTap = { mount(container, opts) { ... return handle; } }` where
 * `handle` exposes at least `{ getDoc(), setDoc(doc), save(), destroy() }` (this file's fallback's shape).
 * `LotekReportingEditor.mount()` below already prefers it when present — no other file changes.
 */
...
(function (window, document) {
  ...
  // ---------------------------------------------------------------------------- schema constants
  // The ProseMirror node/mark schema. scribble mirrors this in scribble/content/schema.py (a frozen
  // contract — the two must not drift); a consuming extension serializes/deserializes the same JSON.
  var NODE = {
    DOC: "doc",
    PARAGRAPH: "paragraph",
    TEXT: "text",
  ...
  // successfully with no newer edit pending behind it. LotekReportingOutbox (reporting-outbox.js)
  ...
  function armPendingGuard(state) {
    if (window.LotekReportingOutbox) window.LotekReportingOutbox.setExternalPending(pendingKey(state), true);
  }
  ...
  function saveNow(state) {
    ...
    fetch(state.apiBase + "/findings/" + state.findingId + "/blocks/" + encodeURIComponent(state.block), {
      ...
      .catch(function (err) {
        console.error("[lotek-reporting-editor] autosave failed", err);
  ...
  // per block), and LotekReportingOutbox's events are global, so a per-instance registration would fire the
  ...
  if (window.LotekReportingOutbox && !window.__lotekReportingEditorOutboxWired) {
    window.__lotekReportingEditorOutboxWired = true;
  ...
  function getAnonUser() {
    try {
      var key = "lotek-reporting-anon-user";
  ...
  function readEmbeddedDoc(container) {
    var script = container.querySelector("script.lotek-reporting-editor-doc-data");
  ...
  function readVariableKeys(container) {
    var script = container.querySelector("script.lotek-reporting-editor-vars-data");
    if (!script) return null;
    try {
      return JSON.parse(script.textContent || "null");
    } catch (e) {
  ...
  function mount(container, options) {
    if (!container) throw new Error("LotekReportingEditor.mount: container element required");
    options = options || {};
    ...
    var opts = {
      ...
      // No default: the kit primitive assumes no extension. The host supplies `apiBase` (data-api-base)
      // — scribble passes "/scribble/api" from its template — or explicit `uploadUrl` etc. later.
      apiBase: options.apiBase || ds.apiBase || "",
    ...
    if (window.LotekReportingEditorTipTap && typeof window.LotekReportingEditorTipTap.mount === "function") {
      return window.LotekReportingEditorTipTap.mount(container, opts);
    }
    return mountFallbackEditor(container, opts);
  }

  window.LotekReportingEditor = {
    mount: mount,
