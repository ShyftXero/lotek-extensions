/*!
 * lotek_kit/static/reporting-outbox.js — the SHARED, dependency-free, IndexedDB-backed upload outbox
 * for lotek's reporting surfaces. Extracted from scribble's original (WS14 client resilience,
 * PLAN.md §19 "don't lose in-progress work (crash / offline)") into the kit so every extension that
 * uploads reporting evidence uses ONE implementation, not a per-extension copy.
 *
 ...
 *
 * Contract (consumed by lotek_kit/static/reporting-editor.js and any extension's paste-upload wiring):
 *
 *   var tempId = LotekReportingOutbox.enqueueUpload({
 *     tempId: "optional-caller-chosen-id",   // generated if omitted; returned either way
 *     url: "/<ext>/.../upload",               // POST target -- the CALLER supplies its own endpoint
 *     blob: fileOrBlob,
 *     filename: "screenshot.png",
 *     fields: { engagement_id: "1", finding_id: "2", caption: "..." },  // extra multipart fields
 *   });
 *
 *   LotekReportingOutbox.on("resolved", function (tempId, serverJson, op) { ... });  // 2xx: op succeeded
 *   LotekReportingOutbox.on("failed",   function (tempId, error, op) { ... });      // 4xx: dropped, won't retry
 *   LotekReportingOutbox.pendingCount() -> number                                   // outbox + external pending
 *   LotekReportingOutbox.isGuardArmed() -> boolean                                  // beforeunload currently armed
 *   LotekReportingOutbox.setExternalPending(key, isPending)  // let other subsystems (e.g. editor.js's text
 *                                                       // autosave) share this module's beforeunload guard
 *   LotekReportingOutbox.flush() -> Promise<void>            // re-attempt every durably-queued pending op now
 *
...
(function (window) {
  ...
  if (window.LotekReportingOutbox) return;

  var DB_NAME = "lotek-reporting-outbox";
  var DB_VERSION = 1;
  var STORE = "uploads";
  var LOCK_NAME = "lotek-reporting-outbox-flush";

  // Retry/backoff tunables. Defaults are the production values; a page may override them by setting
  // `window.__lotekReportingOutboxConfig` BEFORE this script loads (used by the e2e suite to exercise the
  // attempt cap without waiting out the full production backoff schedule).
  var _cfg = (window && window.__lotekReportingOutboxConfig) || {};
    ...
    (_listeners[event] || []).slice().forEach(function (fn) {
      try {
        ...
        if (window.console && window.console.error) {
          window.console.error("[lotek-reporting-outbox] listener error", e);
        }
      }
    });
  }
