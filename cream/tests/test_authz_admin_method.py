...
treat EVERY logged-in user as an admin. This pins the identity-compare fix in ``cream.deps`` — mirrors
``bugreport/tests/test_authz.py::test_an_is_admin_METHOD_does_not_make_everyone_an_admin``.
"""
...
from cream.deps import current_actor_is_admin
...
def test_an_is_admin_method_does_not_make_everyone_an_admin(app, hooks):
    hooks["actor"] = _MethodUser()
