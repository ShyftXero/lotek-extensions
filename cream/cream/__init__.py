"""CREAM — a mountable client quoting + invoicing package.
...
No payment processing — CREAM generates SOW/quotes + invoices; collection happens off-platform.
"""
...
from cream.db import create_all, make_session_factory

__all__ = ["register", "CreamConfig", "__version__"]
...
def register(
    ...
    **_host_models,
    ...
    """Attach CREAM to ``app`` against ``engine`` and return its :class:`CreamConfig`.

    In lotek: pass the host engine + session factory + ``client_model`` + ``base_template='base.html'``.
    ``**_host_models`` absorbs any extra host models the mount framework passes that CREAM doesn't use.
    """
