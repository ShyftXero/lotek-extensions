"""`fraction/facts.py` — the ONE shape rule table (CONTRACT-FACTS §4.2), one case per shape for BOTH
...
`fraction/facts.py`, and — per CONTRACT-FACTS §7.3 — no tool name in THIS FILE either (asserted below
...
class _Dto:
    ...
    `fraction.facts._ALLOWED_FIELDS` may reference via `{"field": "..."}`."""
