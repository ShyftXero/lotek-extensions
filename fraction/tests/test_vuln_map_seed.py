"""`fraction.seed.seed_vuln_map` — the idempotent `FractionVulnMap` seed
(ported from the deleted lotek `tests/test_api_v1_vulnmap.py`'s builtin-VulnMap-seed section, now
exercised directly against `fraction.seed` instead of through a booted lotek app).
...
from fraction.seed.loader import seed_vuln_map
...
def test_seed_is_idempotent_across_reseed_runs(session_factory):
    ...
    duplicate rows."""
    with session_factory() as db:
        before = db.execute(select(fm.FractionVulnMap)).scalars().all()
        before_ids = {r.id for r in before}
        seed_vuln_map(db)  # re-run
        db.commit()
        after = db.execute(select(fm.FractionVulnMap)).scalars().all()
...
def test_seed_fails_loudly_on_zero_match(session_factory, tmp_path):
    ...
    with (
        ...
        pytest.raises(RuntimeError, match="no active fraction_vuln_templates row named"),
    ):
...
def test_seed_ambiguous_name_picks_lowest_id(session_factory, tmp_path, caplog):
    ...
    with session_factory() as db:
        ...
        db.commit()
        row = db.execute(
            select(fm.FractionVulnMap).where(fm.FractionVulnMap.source == "duptest")
        ).scalar_one()
