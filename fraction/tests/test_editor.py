"""Tests for WS4 Phase A: block autosave (fraction/autosave_api.py) + presence
(fraction/collab/presence.py).

These modules don't touch fraction/api.py, fraction/blueprint.py, or fraction/__init__.py (frozen for
...
shared blueprint singletons and call our own ``register()`` before ``fraction.register()`` runs
...
import fraction
import fraction.collab.presence as presence_module
from fraction import autosave_api
...
def app(tmp_path):
    ...
    # fraction.register() calls app.register_blueprint). Both register() calls are idempotent, so
