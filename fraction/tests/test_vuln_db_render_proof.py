"""Task V proof: the rewritten lotek vuln-DB templates (fraction/seed/lotek_vulnerabilities.json)
...
from fraction.models import Engagement, FindingGroup
from fraction.promote import promote_one
...
def test_asrep_kerberoast_certipy_secretsdump_render_with_real_facts(session_factory):
    ...
    # A matched FractionVulnMap entry promotes with the LIBRARY TEMPLATE's own name (not the raw scan
...
def test_xss_and_kubernetes_render_with_real_facts(session_factory):
    """Two more templates touched by this task, exercised through their real seeded FractionVulnMap
