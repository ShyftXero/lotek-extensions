"""WS2 tests: vulnerability template library CRUD (fraction/library_ui.py).

Fixtures (`app`, `client`, `session_factory`) come from `tests/conftest.py`: `fraction.register()` wires
...
standard fixtures the same way `tests/test_smoke.py` exercises `fraction.library`.
...
from fraction import library_ui
...
from fraction.models import Tag, VulnerabilityTemplate

API_PREFIX = "/fraction/api"
...
def test_library_page_renders(client):
    resp = client.get("/fraction/library")
    assert resp.status_code == 200


def test_library_new_page_renders(client):
    resp = client.get("/fraction/library/new")
    assert resp.status_code == 200
...
def test_library_search_by_name(client, session_factory):
    ...
    resp = client.get("/fraction/library", query_string={"q": "Zzyzx Unique"})
    assert resp.status_code == 200
...
def test_library_search_excludes_non_matching(client, session_factory):
    ...
    resp = client.get("/fraction/library", query_string={"q": "zzAlpha"})
...
def test_library_filter_by_category(client, session_factory):
    ...
    resp = client.get("/fraction/library", query_string={"category": "zzCatA"})
...
def test_library_filter_by_severity(client, session_factory):
    ...
    resp = client.get("/fraction/library", query_string={"severity": "critical"})
...
def test_library_filter_by_tag(client, session_factory):
    ...
    resp = client.get("/fraction/library", query_string={"tag": "zzweb"})
...
def test_library_excludes_inactive_by_default_and_toggle_shows_them(client, session_factory):
    ...
    default_resp = client.get("/fraction/library")
    assert b"zzInactive One" not in default_resp.data

    toggled_resp = client.get("/fraction/library", query_string={"inactive": "1"})
...
def test_create_template_minimal(client, session_factory):
    ...
    assert data["redirect"].endswith(f"/fraction/library/{data['id']}")
