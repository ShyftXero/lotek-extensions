"""Machine API — `FractionVulnMap` CRUD + resolution (`/fraction/machine/vuln-map`,
`/fraction/machine/resolve-template`).
...
def _two_template_ids(client):
    ...
    assert len(items) >= 2, "expected the seeded fraction library to have >=2 templates"
