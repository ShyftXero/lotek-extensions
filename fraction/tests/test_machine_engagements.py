...
`fraction/api_pat.py` mounted at `/fraction/machine/*`.

Ported from the deleted lotek `tests/test_api_v1_fraction.py` (the routes moved off
`/api/v1/fraction/*` in the reporting-decoupling refactor, CONTRACT.md §4). Auth/scope RBAC is the
HOST's own concern (proven against a REAL lotek host in the lotek repo's
`tests/test_extension_machine_prefix.py`); this file proves fraction's OWN logic against the
...
def test_machine_route_503_with_no_host_mounted(client):
    """Standalone Fraction (no `stub_host` wired) has no PAT scheme -- a machine route must refuse,
    never run unauthenticated (`fraction/host.py::_no_host`)."""
...
def test_create_engagement_repoints_client_id_to_injected_host_client(app, session_factory):
    ...
    with session_factory() as db:
        host_client = fm.Client(name="Acme Corp")  # stand-in "host" row on fraction's own table
        db.add(host_client)
        db.commit()
        cid = host_client.id
...
def test_promote_lotek_finding_respects_job_tenancy(client, stub_host, session_factory):
    ...
    decision is entirely the HOST's (`host.findings().get_finding`), and `fraction/api_pat.py` never
