"""WS13 tests: AssessmentType admin UI + API (fraction/assessment_types_ui.py).

Self-contained: wires ``assessment_types_ui.register(api_bp, bp)`` onto the *shared* blueprint
singletons before ``fraction.register()`` attaches them to any app -- exactly what the driver will
eventually do inside ``fraction/__init__.py::_wire_feature_routes`` (see tests/test_artifacts.py for the
...
first ``fraction.register()`` call of the whole process, before any blueprint is attached to any app),
...
some other test module's ``app`` fixture calls plain ``fraction.register()`` at *test setup* time. Since
...
import fraction
from fraction import assessment_types_ui
from fraction.api import api_bp
from fraction.blueprint import bp
from fraction.models import AssessmentType, Client, Engagement, FindingGroup
...
def test_assessment_types_page_renders(client):
    resp = client.get("/fraction/assessment-types")
    assert resp.status_code == 200


def test_page_lists_seeded_defaults(client):
    # fraction/seed/loader.py ships four defaults; the admin page must not hide them.
    resp = client.get("/fraction/assessment-types")
...
def test_deactivate_hides_from_active_query_but_keeps_row(client, session_factory):
    ...
    page = client.get("/fraction/assessment-types")
...
def test_active_row_not_marked_inactive(client, session_factory):
    ...
    body = client.get("/fraction/assessment-types").data.decode()
    marker = f'data-at-id="{type_id}"'
    idx = body.index(marker)
    row_start = body.rfind("<tr", 0, idx)
    row_html = body[row_start : idx + 400]
