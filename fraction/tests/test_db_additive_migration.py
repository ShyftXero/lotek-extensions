"""create_all additively adds new model columns to a pre-existing table (Fraction has no migration
framework, and SQLAlchemy create_all only makes missing TABLES, never new COLUMNS)."""
from __future__ import annotations

from sqlalchemy import create_engine, inspect, text

from fraction.db import create_all


def test_create_all_adds_missing_columns_to_existing_table(tmp_path):
    eng = create_engine(f"sqlite:///{tmp_path / 'm.db'}")
    # simulate a fraction_engagements table from an older schema (missing owner_id, client_id, ...)
    with eng.begin() as c:
        c.execute(text("CREATE TABLE fraction_engagements (id INTEGER PRIMARY KEY, name VARCHAR)"))
    create_all(eng)  # must additively add the new columns, not choke on the existing table
    insp = inspect(eng)
    cols = {col["name"] for col in insp.get_columns("fraction_engagements")}
    ...
    indexed = {c for idx in insp.get_indexes("fraction_engagements") for c in idx["column_names"]}
...
def test_create_all_fresh_db_has_owner_id(tmp_path):
    ...
    create_all(eng)
    cols = {col["name"] for col in inspect(eng).get_columns("fraction_engagements")}
