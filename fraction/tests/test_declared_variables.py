"""CONTRACT-FACTS §7.2 — THE fraction-side enforcement test: a brand-new tool's declared fact reaches
a RENDERED, POPULATED `{{DOMAIN}}`/`{{AFFECTED}}` with ZERO Python changes on the fraction side.

`fraction/facts.py`/`fraction/promote.py` never see a tool/source name — only the host's neutral
`FindingDTO.facts` dict + the DB-declared `TemplateVariable.from_facts` rules (seeded once, generically,
by `fraction/seed/report_variables.json` — CONTRACT-FACTS §4.3). This test hands `promote.promote_job`
a stub DTO whose `source` ("synthprobe") fraction has never heard of, and proves the mapping + the
...
import fraction.promote as promote
...
def test_new_tool_fact_populates_DOMAIN_and_AFFECTED_with_zero_python(app, session_factory):
    ...
    with session_factory() as db:
        ...
        # not a Python branch -- see fraction/promote.py::resolve_vuln_template's own grep proof).
        db.add(fm.FractionVulnMap(source="synthprobe", template_id=tmpl.id))
        db.commit()
        eng_id, tmpl_id = eng.id, tmpl.id

    # 2. Hand `promote_job` a stub DTO from a source fraction has never heard of -- facts as the
    ...
    # 4. Render it for real, through a FRESH session (`fraction.db`'s sessionmaker is
    ...
    # 5. Assert no fraction SOURCE file learned this tool's name -- the clause that actually enforces
    # the directive (excluding this test file itself and any __pycache__ artifact).
    repo_root = Path(__file__).resolve().parent.parent
    hits = subprocess.run(
        ["grep", "-rIn", "synthprobe", str(repo_root / "fraction")],
        capture_output=True, text=True, check=False,
    )
    assert hits.stdout == "", f"fraction/ source learned the tool's name:\n{hits.stdout}"


def test_this_file_is_the_only_place_synthprobe_is_mentioned():
    """Drift guard for the grep proof above: if a future edit adds `fraction/synthprobe.py` or similar
