...
Boots the real Fraction app on a background werkzeug thread (same pattern as
...
This guards ``fraction/static/outbox.js`` (the IndexedDB-backed upload outbox) end to end:
...
- the shared beforeunload guard (``FractionOutbox.isGuardArmed()`` / ``pendingCount()``) is armed for
...
_ARTIFACTS_ROUTE = "**/fraction/api/artifacts"
...
def live_app(tmp_path_factory):
    """Boot a real Fraction app (demo-seeded + one dedicated, artifact-free finding for these tests)
    on a background werkzeug server, and tear it down after the module's tests finish."""
    tmp = tmp_path_factory.mktemp("fraction-e2e-resilience")
    ...
    with cfg.session_factory() as session:
        ...
        # A dedicated group/finding with NO pre-attached evidence, so any `.fraction-gallery-item`
...
def _upload_via_gallery(page, gallery, png_path: Path) -> None:
    gallery.locator(".fraction-gallery-file").set_input_files(str(png_path))
    gallery.locator(".fraction-gallery-upload button[type=submit]").click()
...
def _real_item_by_filename(gallery, filename: str):
    ...
    ``.fraction-gallery-item[data-id]`` locator matches *any* of them -- scope to this test's own
    filename so "the real row landed" can't be satisfied by a different test's leftover artifact."""
    return gallery.locator(".fraction-gallery-item[data-id]", has_text=filename)
...
def test_upload_retries_on_transient_failure_then_persists(page, live_app, tmp_path):
    ...
    try:
        ...
        pending = gallery.locator(".fraction-gallery-item.is-pending")
        ...
        assert gallery.locator(".fraction-gallery-item.is-pending").count() == 0
        assert gallery.locator(".fraction-gallery-item.is-failed").count() == 0
...
def test_upload_survives_reload_while_offline_then_flushes(page, live_app, tmp_path):
    ...
    try:
        ...
        gallery.locator(".fraction-gallery-item.is-pending").first.wait_for(
...
def test_beforeunload_guard_armed_while_pending_and_cleared_after_flush(page, live_app, tmp_path):
    ...
    observable state FractionOutbox exposes instead: pendingCount()/isGuardArmed() must be truthy
    ...
    try:
        ...
        assert page.evaluate("window.FractionOutbox.pendingCount()") == 0
        assert page.evaluate("window.FractionOutbox.isGuardArmed()") is False
        ...
        assert page.evaluate("window.FractionOutbox.pendingCount()") > 0
        assert page.evaluate("window.FractionOutbox.isGuardArmed()") is True
...
def test_editor_inline_paste_transient_fail_then_persists_with_real_artifact_id(page, live_app, tmp_path):
    ...
    try:
        ...
        editor = page.locator(f'.fraction-editor-wrap[data-block="{block}"]')
...
def test_outbox_gives_up_after_max_attempts_and_drains(page, live_app, tmp_path):
    ...
    page.add_init_script(
        "window.__fractionOutboxConfig = { maxAttempts: 3, baseDelayMs: 20, maxDelayMs: 40 };"
    )
    finding_id = live_app["finding_id"]
    ...
    try:
        ...
        gallery.locator(".fraction-gallery-item.is-failed").first.wait_for(
            state="attached", timeout=10000
        )
        # The whole point of the cap: the queue drains and the guard clears.
        assert _poll(page, lambda: page.evaluate("window.FractionOutbox.pendingCount()") == 0)
        assert page.evaluate("window.FractionOutbox.isGuardArmed()") is False
