...
``fraction/models.py::Engagement``): standalone it points at ``fraction_clients``; mounted, it should
point at the HOST's own client table instead, with nothing ever written to ``fraction_clients``. These
...
from fraction.models import Client, Engagement, EngagementFinding
...
def app(tmp_path):
    ...
    # The host's own tables live on the SAME engine Fraction mounts against (mirrors Lotek: one shared
    # SQLite DB, Fraction's fraction_* tables additive alongside Lotek's own).
    _HostBase.metadata.create_all(engine)

    cfg = fraction.register(
        flask_app, engine, instance_path=str(tmp_path), base_template="fraction/base.html"
    )
...
def session_factory(app):
    return app.extensions["fraction"].session_factory


# ------------------------------------------------------------------------- client_model: standalone


def test_engagement_create_resolves_fractions_own_client_standalone(client, session_factory):
    """No client_model injected (the default): engagement_new's select-or-create writes to
    fraction_clients, and Engagement.resolve_client reads it back -- the pre-existing behavior,
...
def test_engagement_create_repoints_to_injected_host_client_model(client, session_factory, app):
    ...
    fraction_clients), and Engagement.resolve_client reads a HostClient back -- proving a real repoint,
    not a fraction_clients shadow sync (docs/LOTEK_ADOPTION.md §3.1, "option a", the clean end-state)."""
    cfg = app.extensions["fraction"]
    ...
    try:
        ...
        with app.app_context(), session_factory() as db:
            ...
            # The real proof this is a REPOINT, not a shadow table: nothing landed in fraction_clients,
...
def test_dashboard_and_health_client_counts_reflect_injected_host_model(client, session_factory, app):
    ...
    fraction.models.Client to client_model() so they don't silently read an always-empty
    fraction_clients table once client creation has moved to the host's table."""
    cfg = app.extensions["fraction"]
    cfg.client_model = HostClient
    try:
        with session_factory() as db:
...
def test_from_lotek_finding_uses_fractions_own_severity_standalone(app):
...
def test_from_lotek_finding_uses_injected_host_severity_when_mounted(app):
    ...
    try:
        ...
        with app.app_context():
            finding = EngagementFinding.from_lotek_finding(fake_finding)
        # Value-identical (docs/LOTEK_ADOPTION.md §3.2) so this still equals fraction's own Severity.high
...
def test_from_lotek_finding_defaults_safely_with_no_app_context_at_all():
    """No Flask app pushed at all (e.g. a script driving fraction.models directly): severity_enum()'s
    RuntimeError guard falls back to fraction.enums.Severity rather than raising."""
