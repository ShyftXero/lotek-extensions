"""Tests for the fraction.deps host-hook accessors added for Lotek adoption (docs/LOTEK_ADOPTION.md §4):
...
``FractionConfig.extras`` (fraction/config.py:25). Absent hook, a hook returning ``None``/an object with
...
``client_model``/``severity_enum`` instead wrap a plain ``FractionConfig`` field (not an ``extras``
callable): the host passes a MODEL/ENUM directly to ``register(..., client_model=..., severity_enum=...)``
rather than a hook function, so these resolvers just return ``cfg.client_model or <fraction's own>`` /
``cfg.severity_enum or <fraction's own>`` -- see fraction/deps.py.
"""
...
from fraction.models import Client

# --------------------------------------------------------------------------------- current_actor_username


def test_current_actor_username_none_without_host_hook(app):
    # Standalone Fraction (no host, no extras entry): always None.
...
def test_current_actor_id_none_without_host_hook(app):
    # Standalone Fraction: no host actor -> owner_id attribution is None.
    with app.app_context():
...
def test_current_actor_username_none_when_hook_raises(app):
    ...
    try:
        with app.app_context():
            ...
            # otherwise turn a host-side outage into a 500 on every Fraction write.
            assert current_actor_username() is None
    finally:
        cfg.extras.pop("current_actor", None)
...
def test_host_can_write_true_without_host_hook(app):
    # Standalone Fraction is always writable.
    with app.app_context():
...
class _HostClient:
    ...
    distinct object identity from ``fraction.models.Client`` for these resolver-level tests; nothing
    here touches a database."""


def test_client_model_defaults_to_fractions_own_standalone(app):
    # Standalone Fraction (register() called with no client_model, as tests/conftest.py's `app` fixture
    # does): the resolver falls back to fraction.models.Client.
    with app.app_context():
...
def test_severity_enum_defaults_to_fractions_own_standalone(app):
    with app.app_context():
...
def test_severity_enum_returns_the_injected_host_enum_when_mounted(app):
    class _HostSeverity(enum.StrEnum):
        """Value-identical to fraction.enums.Severity (docs/LOTEK_ADOPTION.md §3.2) but a DIFFERENT
