...
`fraction.engagement_ui.register(api_bp, bp)` is already wired unconditionally by
`fraction/__init__.py:_wire_feature_routes`, so the plain `app`/`client`/`session_factory` fixtures from
...
`fraction_can_write` context-processor value (+ its effect on the rendered board/create-engagement HTML)
...
from fraction.blueprint import _inject_base
from fraction.content import schema
from fraction.enums import Confidence, FindingStatus, OrderMode, Severity
...
def test_finding_detail_get_renders_editor_and_gallery(client, session_factory):
    ...
    assert "fraction-gallery" in body
...
# ------------------------------------------------------------------------------- fraction_can_write gating


def test_fraction_can_write_defaults_true_without_host_hook(app):
    with app.app_context():
        assert _inject_base()["fraction_can_write"] is True


def test_fraction_can_write_reflects_host_can_write_hook(app):
    cfg = app.extensions["fraction"]
    cfg.extras["can_write"] = lambda: False
    try:
        with app.app_context():
            assert _inject_base()["fraction_can_write"] is False
    finally:
...
def test_board_hides_mutating_controls_for_read_only_viewer(client, session_factory, app):
    ...
    # e.g. `.fraction-board-group-delete { margin: 0; }` unconditionally -- asserting on a CSS class
