"""Report route authorization (`fraction/report_html_api.py::_authorize_engagement_view`).

Ported from the deleted lotek `tests/test_fraction_report_authz.py` (adversarial review 2026-07-27,
...
the id). The check itself lives entirely in fraction (`cfg.extras['host']` truthy -> enforce; admins
...
def test_standalone_no_host_applies_no_authorization(client, session_factory):
    """Without `stub_host` wired (`cfg.extras['host']` absent), standalone Fraction has no host
