"""Tests for WS11 Phase B: server-side CRDT co-editing (fraction/collab/crdt.py + pm_yjs.py).
...
from fraction.collab import crdt, pm_yjs
from fraction.content.render_html import render_block
from fraction.models import CollabDoc, Engagement, EngagementFinding
...
# will (before any fraction.register() call registers `bp`/`api_bp` on a Flask app — Flask forbids
# `.route()` after a blueprint has been registered once). Unlike the already-integrated WS4 hooks
# (autosave/presence, wired into `fraction/__init__.py:_wire_feature_routes` by the driver, which
# guarantees they run before the *first* `fraction.register()` call of the whole test session no matter
# which test file happens to run first), `crdt.register` isn't wired in yet — integrating it is the
# driver's job (see this workstream's rails: do not edit `fraction/__init__.py`). So this module-level
...
def _isolated_manager(monkeypatch):
    """``fraction.collab.crdt.manager`` is a module-level singleton (mirrors
    ``fraction.collab.presence.registry``); swap in a fresh one per test so rooms from one test never
...
def test_register_is_idempotent_and_mounts_collab_status(client, finding_id):
    ...
    resp = client.get(f"/fraction/api/findings/{finding_id}/blocks/{BLOCK}/collab-status")
