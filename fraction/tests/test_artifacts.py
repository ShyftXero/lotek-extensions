...
tests/conftest.py) so it can wire ``fraction.artifacts_api.register(api_bp, bp)`` onto the *shared*
blueprint singletons before ``fraction.register()`` attaches them to the app — mirroring exactly what
the driver will eventually do inside ``fraction/__init__.py``. ``register()`` is idempotent, so this is
...
from fraction.artifacts_api import artifact_url
from fraction.artifacts_api import register as register_artifacts
from fraction.artifacts_storage import guess_content_type, resolve_path, safe_join, save_bytes
from fraction.blueprint import bp
from fraction.enums import ArtifactPlacement, Severity
...
@pytest.fixture
def app(tmp_path):
    # Wires the WS5 hook onto the shared blueprint singletons *before* fraction.register() attaches
...
def test_health_still_works_after_artifacts_registered(client):
    ...
    resp = client.get("/fraction/api/health")
...
def test_raw_download_is_forced_attachment(client, session_factory):
    ...
    resp = client.get(f"/fraction/api/artifacts/{artifact_id}/raw")
    assert resp.status_code == 200
...
def test_raw_download_missing_returns_404(client):
    resp = client.get("/fraction/api/artifacts/999999/raw")
...
def test_update_caption_and_include_toggle(client, session_factory):
    ...
    resp = client.post(
        f"/fraction/api/artifacts/{artifact_id}",
...
def test_update_missing_artifact_404(client):
    resp = client.post("/fraction/api/artifacts/999999", json={"caption": "x"})
    assert resp.status_code == 404
...
def test_reorder_persists_order_index(client, session_factory):
    ...
    resp = client.post(f"/fraction/api/findings/{finding_id}/artifacts/reorder", json={"order": new_order})
    assert resp.status_code == 200

    listing = client.get(f"/fraction/api/findings/{finding_id}/artifacts").get_json()
...
def test_reorder_requires_order_list(client, session_factory):
    ...
    resp = client.post(f"/fraction/api/findings/{finding_id}/artifacts/reorder", json={"order": "nope"})
    assert resp.status_code == 400
...
def test_delete_removes_row_and_file(client, session_factory, cfg):
    ...
    resp = client.post(f"/fraction/api/artifacts/{artifact_id}/delete")
    assert resp.status_code == 200
...
def test_delete_missing_artifact_404(client):
    resp = client.post("/fraction/api/artifacts/999999/delete")
    assert resp.status_code == 404
...
def test_list_finding_artifacts_ordered(client, session_factory):
    ...
    resp = client.get(f"/fraction/api/findings/{finding_id}/artifacts")
    assert resp.status_code == 200
    body = resp.get_json()
...
def test_artifact_url_builds_raw_endpoint(app):
    ...
    assert url.startswith("/fraction/api")
