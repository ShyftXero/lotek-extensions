...
def app(tmp_path):
    ...
    cfg = fraction.register(app, engine, instance_path=str(tmp_path), base_template="fraction/base.html")
...
def clean_vuln_map(session_factory):
    """Purge the builtin `FractionVulnMap` seed (`fraction.seed.seed_vuln_map`, applied by the base
    ...
    from fraction.models import FractionVulnMap

    with session_factory() as db:
        db.execute(delete(FractionVulnMap))
        db.commit()
...
# `mark_job_promoted`) with fakes, so fraction's OWN tests can exercise its machine blueprint
# (`fraction/api_pat.py`), its report authz (`report_html_api._authorize_engagement_view`), and its
...
# `require_pat_scope`/`pat_authenticate` here are deliberately NO-OP passthroughs: fraction's tests
# exist to prove fraction's OWN logic (tenancy pass-through, promote/aggregation, facts→variable
...
@dataclass
class StubUser:
    """The BROWSER-session identity (`fraction.deps.current_actor()`), distinct from `StubActor` (the
