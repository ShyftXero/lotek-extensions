"""Authors ``fraction/report_templates/default.docx`` — the docxtpl template WS8 renders against.

Run this script to (re)generate the committed ``default.docx`` binary whenever the template layout
needs to change::

    uv run python -m fraction.report_templates.build_default_docx
...
# Matches fraction/static/fraction.css's --sev-* ramp / render_html.py's CSS (kept in sync by hand —
