...
`fraction/__init__.py:_wire_feature_routes` (frozen file, not touched here).

Routes added
------------
UI (``bp``, mounted at the host ``url_prefix``, default ``/fraction``):
...
``current_actor`` hook (``fraction.deps.current_actor_username``) -- ``None`` standalone.
"""
...
from fraction.artifacts_storage import delete_file
...
from fraction.enums import Confidence, FindingStatus, OrderMode, severity_rank
...
def register(api_bp, bp) -> None:
    ...
    def engagements():
        with open_session() as db:
            ...
            return render_template(
                "fraction/engagements.html", engagements=rows, client_names=client_names(db, rows)
            )
    ...
    def engagement_new():
        ...
        with open_session() as db:
            if request.method == "POST":
                ...
                db.add(engagement)
                db.commit()
                return redirect(url_for("fraction.engagement_board", engagement_id=engagement.id))

            clients = db.scalars(select(ClientModel).order_by(ClientModel.name)).all()
            return render_template("fraction/engagement_new.html", clients=clients, error=None)
    ...
    def engagement_edit_page(engagement_id: int):
        ...
        with open_session() as db:
            ...
            clients = db.scalars(select(ClientModel).order_by(ClientModel.name)).all()
            return render_template(
                "fraction/engagement_edit.html", engagement=engagement, clients=clients, error=None
            )
    ...
    def engagement_edit(engagement_id: int):
        ...
        with open_session() as db:
            ...
            db.commit()
        return redirect(url_for("fraction.engagements"))
    ...
    def create_group(engagement_id: int):
        with open_session() as db:
            ...
            if name:
                ...
                db.add(group)
                db.commit()
        return redirect(url_for("fraction.engagement_board", engagement_id=engagement_id))
    ...
    def finding_detail(finding_id: int):
        with open_session() as db:
            ...
            if request.method == "POST":
                ...
                db.commit()
                return redirect(url_for("fraction.finding_detail", finding_id=finding_id))
            ...
            return render_template(
                ...
                fraction_variable_keys=variable_keys,
            )
