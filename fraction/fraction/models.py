"""Fraction data model (SQLAlchemy 2.0 declarative).
...
(see ``fraction.content.schema``); ``content_html`` caches the sanitized rendered HTML.
"""
...
from fraction.db import Base, TimestampMixin
...
class Client(Base, TimestampMixin):
    ...
    Used as the DEFAULT client model standalone (see ``fraction.deps.client_model``); when mounted with
    ...
    __tablename__ = "fraction_clients"
...
class Engagement(Base, TimestampMixin):
    ...
    __tablename__ = "fraction_engagements"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    # Soft reference, NOT a foreign key (docs/LOTEK_ADOPTION.md §3.1): may point at ``fraction_clients``
    # (standalone) or the host's own client table (mounted, e.g. Lotek's ``clients``) -- a static FK/
    # relationship can only target one table, so resolution goes through ``fraction.deps.client_model``
    # at read/write time instead (see ``resolve_client`` below). Same pattern as
    # ``EngagementFinding.asset_id`` below.
    #
    # Schema-history note: ``fraction.db.create_all`` is additive-only (``Base.metadata.create_all``) --
    # it does not retrofit a table that already exists on disk. A ``fraction_engagements`` table created
    # by a PRE-existing checkout (which had ``client_id`` as a real FK to ``fraction_clients.id``) keeps
    ...
    # (no alembic, no ``fraction/migrations.py``) to retrofit an existing file automatically -- drop/
    # recreate the local dev DB (e.g. ``instance/fraction.db``) if you hit a stale constraint.
    ...
    def resolve_client(self, session):
        ...
        table. ``client_id`` is a soft reference that may point at ``fraction_clients`` (standalone) or
        the host's client table (mounted) -- which one is live is only known via
        ``fraction.deps.client_model()`` at call time, so this does a plain ``session.get`` against
        whatever that resolves to instead of a relationship traversal.
        """
        if self.client_id is None:
            return None
        from fraction.deps import client_model  # local import: avoids a models.py <-> deps.py cycle
...
class AssessmentType(Base, TimestampMixin):
    ...
    __tablename__ = "fraction_assessment_types"
...
class FindingGroup(Base, TimestampMixin):
    ...
    __tablename__ = "fraction_finding_groups"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    engagement_id: Mapped[int] = mapped_column(ForeignKey("fraction_engagements.id"))
    assessment_type_id: Mapped[int | None] = mapped_column(ForeignKey("fraction_assessment_types.id"))
...
class VulnerabilityTemplate(Base, TimestampMixin):
    ...
    __tablename__ = "fraction_vuln_templates"
    ...
    tags: Mapped[list[Tag]] = relationship(secondary="fraction_template_tags")


class FractionVulnMap(Base, TimestampMixin):
    ...
    Formerly lotek core's ``VulnMap`` (``vuln_template_map``, a soft/no-FK reference because Fraction
    was an optional extension that might not be mounted). Now that promotion is entirely Fraction's own
    concern -- the host only ever hands over neutral ``FindingDTO``s (see ``src/app/host_contract.py``)
    -- this table lives here instead, with a REAL foreign key: Fraction always has its own
    ``fraction_vuln_templates`` table by definition. Resolution is most-specific-first: ``dedupe_prefix``
    (prefix of the lotek finding's ``dedupe_key``) beats ``source`` + ``title_pattern`` (glob on the
    finding's ``title``) beats ``source`` alone -- see ``fraction/promote.py::resolve_vuln_template``.
    """

    __tablename__ = "fraction_vuln_map"
    ...
    template_id: Mapped[int] = mapped_column(ForeignKey("fraction_vuln_templates.id"), nullable=False)
...
class EngagementFinding(Base, TimestampMixin):
    ...
    __tablename__ = "fraction_findings"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    engagement_id: Mapped[int] = mapped_column(ForeignKey("fraction_engagements.id"))
    group_id: Mapped[int | None] = mapped_column(ForeignKey("fraction_finding_groups.id"))
    template_id: Mapped[int | None] = mapped_column(ForeignKey("fraction_vuln_templates.id"))
    ...
    # standalone Fraction has no Lotek ``Finding`` table to reference at all.
    ...
    parent_id: Mapped[int | None] = mapped_column(ForeignKey("fraction_findings.id"), nullable=True)
    ...
    # promote step from the host's FindingDTO.facts (fraction/facts.py) and by deterministic group-fact
    ...
    # emits neutral facts. Additive column (``fraction.db.create_all`` retrofits it with ``ALTER TABLE
    ...
    tags: Mapped[list[Tag]] = relationship(secondary="fraction_finding_tags")
    ...
    def from_lotek_finding(cls, finding, **overrides) -> EngagementFinding:
        ...
        Constructs the severity via ``fraction.deps.severity_enum()`` so a mounted host's own
        ``Severity`` is used when injected (docs/LOTEK_ADOPTION.md §3.2) -- falls back to Fraction's own
        ``fraction.enums.Severity`` standalone; the two vocabularies are value-identical, so this is
        ...
        """
        from fraction.content.schema import doc_from_text
        from fraction.deps import severity_enum  # local import: avoids a models.py <-> deps.py cycle
...
class Artifact(Base, TimestampMixin):
    ...
    __tablename__ = "fraction_artifacts"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    engagement_id: Mapped[int] = mapped_column(ForeignKey("fraction_engagements.id"))
    finding_id: Mapped[int | None] = mapped_column(ForeignKey("fraction_findings.id"))
    ...
    # constraint, because the additive migration in fraction/db.py can only retrofit plain indexes onto
...
class TemplateVariable(Base, TimestampMixin):
    ...
    __tablename__ = "fraction_variables"
    ...
    # ``shape`` = how to render it AND how to combine children into a parent (fraction/facts.py's ONE
...
class VariableValue(Base, TimestampMixin):
    ...
    __tablename__ = "fraction_variable_values"
    __table_args__ = (UniqueConstraint("variable_id", "engagement_id", "finding_id"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    variable_id: Mapped[int] = mapped_column(ForeignKey("fraction_variables.id"))
    engagement_id: Mapped[int | None] = mapped_column(ForeignKey("fraction_engagements.id"))
    finding_id: Mapped[int | None] = mapped_column(ForeignKey("fraction_findings.id"))
...
class FindingTag(Base):
    __tablename__ = "fraction_finding_tags"

    finding_id: Mapped[int] = mapped_column(ForeignKey("fraction_findings.id"), primary_key=True)
    tag_id: Mapped[int] = mapped_column(ForeignKey("fraction_tags.id"), primary_key=True)


class TemplateTag(Base):
    __tablename__ = "fraction_template_tags"

    template_id: Mapped[int] = mapped_column(ForeignKey("fraction_vuln_templates.id"), primary_key=True)
    tag_id: Mapped[int] = mapped_column(ForeignKey("fraction_tags.id"), primary_key=True)
...
class ReportTemplate(Base, TimestampMixin):
    __tablename__ = "fraction_report_templates"
...
class ReportRender(Base, TimestampMixin):
    __tablename__ = "fraction_report_renders"
...
class CollabDoc(Base, TimestampMixin):
    ...
    __tablename__ = "fraction_collab_docs"
    __table_args__ = (UniqueConstraint("finding_id", "block"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    finding_id: Mapped[int] = mapped_column(ForeignKey("fraction_findings.id"))
...
# for its side effect so create_all() (which imports fraction.models) creates it. See fraction/enrichment.py.
from fraction import enrichment  # noqa: E402, F401  (side-effect registration)
