...
Fraction owns its own declarative ``Base`` when standalone. When mounted in Lotek, ``register()`` is
handed the host engine/session factory and simply creates Fraction's tables in the shared database; the
...
def create_all(engine) -> None:
    """Create Fraction's tables, then additively add any new columns (and their indexes) to
    pre-existing tables.
    ...
    Fraction has no migration framework, so when a model gains a column (e.g. ``Engagement.owner_id``) a
    ...
    from fraction import models  # noqa: F401
    ...
    with engine.begin() as conn:
        for table in Base.metadata.sorted_tables:
            ...
            if added:
                ...
                # (e.g. ix_fraction_engagements_owner_id) is missing. checkfirst -> IF NOT EXISTS.
