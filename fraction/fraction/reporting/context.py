...
from fraction.enums import OrderMode, Severity, risk_rating, severity_rank
from fraction.templating import build_context, build_full_context, make_var_resolver
...
class FindingCtx:
    ...
    # see ``fraction.facts``/``fraction.promote``). Also the source of ``facts_line`` (a tight per-host
...
def _facts_line(variables: dict) -> str:
    ...
    report-variable overlay (``EngagementFinding.variables`` — see ``fraction.facts``/``fraction.promote``),
...
def _finding_ctx(finding, *, artifact_url) -> FindingCtx:
    ...
    blocks_html = {}
    from fraction.templating import resolve_doc  # local import avoids cycle at module import
...
def build_report_context(engagement, *, artifact_url=None) -> ReportContext:
    ...
    # docs/LOTEK_ADOPTION.md §3.1 / fraction.models.Engagement.resolve_client). Resolve through the
