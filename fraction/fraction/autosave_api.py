...
(a dict keyed by block name — see ``fraction/content/schema.py``). This module adds the block
read/write endpoints on top of that: the client (``fraction/static/editor.js``) debounces edits and
PUTs/POSTs the current doc; the server validates it's a ProseMirror ``doc``, stores it, and re-derives +
caches sanitized HTML via ``fraction.content.render_html.render_block`` into ``content_html`` so list
views/previews never need to walk JSON themselves.

This module does **not** import or edit ``fraction/api.py`` / ``fraction/blueprint.py`` /
``fraction/__init__.py`` (frozen; see ``plans/CONTRACTS.md`` ownership map). Instead it exposes
``register(api_bp, bp)``, which the driver calls (from the mount path, before the blueprints are
registered on the host app) to attach these routes onto the shared ``fraction_api`` blueprint object.
...
from fraction.deps import get_config, open_session
from fraction.models import EngagementFinding
...
def register(api_bp, bp) -> None:
    ...
    ``fraction_api_prefix`` and ``fraction_variable_keys`` to templates, so
    ``fraction/templates/fraction/_editor.html`` can build API URLs and offer a built-in variable list
    ...
    def _inject_editor_globals():
        ...
        from fraction.templating.resolver import BUILTIN_KEYS

        return {"fraction_api_prefix": api_prefix, "fraction_variable_keys": list(BUILTIN_KEYS)}
