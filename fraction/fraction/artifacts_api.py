...
Exposes :func:`register`, which the driver calls once from ``fraction/__init__.py`` — after the API
...
This module deliberately does not import or edit ``fraction/api.py`` / ``fraction/blueprint.py`` /
``fraction/__init__.py`` (frozen/shared files); it only adds routes to the blueprint objects handed to
it, exactly like ``fraction/blueprint.py`` route functions do with ``@bp.get(...)``.
...
``POST /artifacts`` also accepts:
    ...
    ``idempotency_key``  -- dedup token for retried uploads (the offline upload-outbox story, PLAN.md
                            ...
                            unique constraint (see ``Artifact.idempotency_key`` in fraction/models.py).
"""
...
from fraction.artifacts_storage import delete_file, guess_content_type, resolve_path, save_bytes
from fraction.deps import current_actor_username, get_config, open_session
from fraction.enums import ArtifactKind, ArtifactPlacement
from fraction.models import Artifact

_REGISTERED_ATTR = "_fraction_artifacts_registered"
...
def artifact_url(artifact_id: int) -> str:
    ...
    """
    return url_for("fraction_api.artifact_raw", artifact_id=artifact_id)
...
def _artifact_dict(a: Artifact) -> dict:
    return {
        ...
        "update_url": url_for("fraction_api.update_artifact", artifact_id=a.id),
        "delete_url": url_for("fraction_api.delete_artifact", artifact_id=a.id),
    }
