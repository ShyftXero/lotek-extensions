...
dangerous: the host (lotek) deliberately does NOT exempt ``/fraction/*`` from CSRF (see
``src/app/extensions.py``'s "security > convenience" docstring, and
``test_fraction_cookie_write_is_csrf_protected``, which pins it). The routes that ARE exempt from CSRF
(and from the cookie-session login gate) live in a separate, disjoint, PAT-only blueprint mounted under
the ``[host] machine_prefix`` the extension declares (``/fraction/machine/*`` -- see
``fraction/api_pat.py``), never here.
"""
...
from fraction.deps import client_model, open_session
    ...
    with open_session() as db:
        return jsonify(
            ...
            counts={
                ...
                # docs/LOTEK_ADOPTION.md §3.1), not always fraction_clients (empty when mounted).
