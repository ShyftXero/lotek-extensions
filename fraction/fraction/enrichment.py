...
write); a human accepts or rejects; accept applies the promotion to the Fraction *report* side and
records the decision. Core scan data stays the scanner's ground truth.

**v2-native table** even though the rest of Fraction is still pre-v2 int-PK: this is a new table, so it
...
class EnrichmentProposal(Base):
    ...
    __tablename__ = "fraction_enrichment_proposals"
...
# ── the driver: a pluggable, read-only vuln lookup (minimal, like fraction/vector) ──────────────────
