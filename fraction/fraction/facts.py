"""fraction/facts.py — the ONE shape rule table interpreting the DB-declared fact -> report-variable
...
in ``fraction_variables`` (``TemplateVariable.from_facts``), not Python keyed on a tool's name. This
...
``tests/test_facts_shapes.py`` in this repo) and safe to call from ``fraction/promote.py`` per finding.
"""
...
from fraction.enums import Severity, severity_rank
...
# Generic host-shape recognizer / URL-scheme prefix. Reimplemented here (not imported) because fraction
...
def _severity_key(text: str) -> int:
    """Rank a raw severity string worst-first (0 = most severe), via ``fraction.enums.severity_rank``.
...
def synthesize_parent_variables(children: Any, declarations: Any) -> dict[str, str]:
    ...
    with ``str()``, severity ties resolve via ``fraction.enums.severity_rank``, no time/randomness/dict-
