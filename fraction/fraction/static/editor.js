/*!
 * Fraction rich-text editor (WS4 Phase A) — fraction/static/editor.js
 * =====================================================================
 *
 * PLAN.md §8 / plans/CONTRACTS.md §3 call for a TipTap/ProseMirror editor vendored as a prebuilt UMD
 * bundle under fraction/static/lib/. This environment cannot reach a package registry/CDN to fetch or
 ...
 * (fraction/content/schema.py) that TipTap would produce. Swapping in a real TipTap bundle later is a
 ...
 *   fraction/content/schema.py: doc/paragraph/text/heading/bulletList/orderedList/listItem/blockquote/
 ...
 * If/when a vendored TipTap bundle lands under fraction/static/lib/ (e.g. `tiptap.bundle.js`), have it
 * define `window.FractionTipTap = { mount(container, opts) { ... return handle; } }` where `opts` is
 ...
 * `FractionEditor.mount()` below already prefers `window.FractionTipTap` when present — no other file
 * needs to change. Load the TipTap bundle script *before* this file's mount() runs (e.g. before the
 * inline auto-mount script in fraction/templates/fraction/_editor.html, or call
 * `FractionEditor.mount(el)` yourself after the bundle is ready).
 */
...
(function (window, document) {
  ...
  // ---------------------------------------------------------------------------- schema constants
  // Mirrors fraction/content/schema.py exactly (frozen contract — do not rename without updating both).
  var NODE = {
    DOC: "doc",
    PARAGRAPH: "paragraph",
    TEXT: "text",
  ...
  // successfully with no newer edit pending behind it. FractionOutbox (fraction/static/outbox.js)
  ...
  function armPendingGuard(state) {
    if (window.FractionOutbox) window.FractionOutbox.setExternalPending(pendingKey(state), true);
  }

  function disarmPendingGuard(state) {
    if (window.FractionOutbox) window.FractionOutbox.setExternalPending(pendingKey(state), false);
  }
  ...
  function saveNow(state) {
    ...
    fetch(state.apiBase + "/findings/" + state.findingId + "/blocks/" + encodeURIComponent(state.block), {
      ...
      .catch(function (err) {
        console.error("[fraction-editor] autosave failed", err);
  ...
  // per block), and FractionOutbox's events are global, so a per-instance registration would fire the
  ...
  if (window.FractionOutbox && !window.__fractionEditorOutboxWired) {
    window.__fractionEditorOutboxWired = true;
  ...
  function readEmbeddedDoc(container) {
    var script = container.querySelector("script.fraction-editor-doc-data");
  ...
  function readVariableKeys(container) {
    var script = container.querySelector("script.fraction-editor-vars-data");
    if (!script) return null;
    try {
      return JSON.parse(script.textContent || "null");
    } catch (e) {
  ...
  function mount(container, options) {
    if (!container) throw new Error("FractionEditor.mount: container element required");
    options = options || {};
    ...
    var opts = {
      ...
      apiBase: options.apiBase || ds.apiBase || "/fraction/api",
    ...
    if (window.FractionTipTap && typeof window.FractionTipTap.mount === "function") {
      return window.FractionTipTap.mount(container, opts);
    }
    return mountFallbackEditor(container, opts);
  }

  window.FractionEditor = {
    mount: mount,
