"""PAT-scoped MACHINE API for Fraction — mounted at ``<url_prefix>/machine`` on its OWN blueprint.

Lets host TOOLS drive Fraction the host's way (``Authorization: Bearer lotek_pat_…`` + scope RBAC)
...
reached through ``fraction.host`` (injected ``extras``), never by importing host modules.

SECURITY (three layers, all fail-closed):
  1. ``machine_bp.before_request = fraction.host.authenticate`` — every route on this blueprint needs a
...
NOT this module's concern — it lives in ``fraction/promote.py`` (imported lazily, inside the two view
...
from fraction import host
from fraction.deps import open_session, severity_enum  # noqa: F401
...
machine_bp = Blueprint("fraction_machine", __name__)
machine_bp.before_request(host.authenticate)


# ── pure helpers (moved verbatim from the deleted src/app/api_v1_fraction.py) ────────────────────────
...
def fraction_create_engagement():
    ...
    with open_session() as db:
        eng = Engagement(
            ...
            # owner_id is unconditional now: fraction owns Engagement/EngagementFinding outright, so it
...
@host.require_scope("read")
def fraction_get_template(template_id: int):
    with open_session() as db:
...
@host.require_scope("write")
def fraction_add_finding(engagement_id: int):
    data = request.get_json(silent=True) or {}
    ...
    with open_session() as db:
        ...
        from fraction.promote import promote_one  # lazy: fraction/promote.py is Track D's file
...
@host.require_scope("read")
def fraction_list_vuln_map():
    with open_session() as db:
        rows = db.execute(select(FractionVulnMap).order_by(FractionVulnMap.id)).scalars().all()
        items = [
            {
                "id": m.id,
                "source": m.source,
...
def fraction_resolve_template():
    ...
    from fraction.promote import resolve_vuln_template  # lazy: fraction/promote.py is Track D's file
...
@host.require_scope("write")
def fraction_promote_job(engagement_id: int, job_id: str):
    """Bulk-promote a lotek scan job's Findings into a Fraction engagement.
    ...
    ``fraction.promote.promote_job``'s concern, not this module's.
    """
    actor = host.actor()
    actor_username = actor.username if actor else None
    with open_session() as db:
        ...
        from fraction.promote import promote_job  # lazy: fraction/promote.py is Track D's file
    ...
    # this is the ONE write the host contract exposes; it is not part of the fraction-side transaction
    ...
    host.mark_job_promoted(job_id, actor, extension="fraction", ref_id=engagement_id)
...
def register(machine_bp_: Blueprint) -> None:  # noqa: ARG001 - routes are already bound above at import
    ...
    ``fraction/api.py`` uses for its own module-level ``api_bp``. Kept as a callable purely so
    ``fraction/__init__.py::_wire_feature_routes`` can invoke every feature hook uniformly.
    """
