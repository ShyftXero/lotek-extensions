...
Two hard constraints force the indirection:
  * ``fraction.register()`` returns its config and the host fills ``cfg.extras`` AFTERWARDS, so a hook
    is not available while routes are being wired.
  * route wiring is a PROCESS singleton (``fraction._wire_feature_routes``) while ``cfg.extras`` is
    PER-APP, so a hook captured at wiring time would be the wrong app's in a multi-app process (tests).

Everything here therefore reads ``fraction.deps.get_config().extras`` at call time and FAILS CLOSED
(503) when the host injected nothing: standalone Fraction has no PAT scheme, so a machine route with no
...
from fraction.deps import get_config
