...
Content-block editing note: `fraction/templates/fraction/_editor.html` + `fraction/static/editor.js`
...
module exposes its own template-scoped block endpoints (mirroring `fraction/autosave_api.py`'s shape:
...
`window.FractionEditor._internal.{docToFragment,domToDoc}` JSON<->DOM converters (explicitly documented
in that file as a reuse point for exactly this kind of adapter) instead of `FractionEditor.mount()`.
...
`api_bp` (JSON). Keep the `library` endpoint name. Wired in `fraction/__init__.py`.
"""
...
from fraction.deps import open_session, severity_enum
        ...
        with open_session() as db:
            ...
            return (
                jsonify(
                    ok=True,
                    id=template.id,
                    redirect=url_for("fraction.library_detail", template_id=template.id),
                ),
                201,
            )
        ...
        with open_session() as db:
            ...
            return (
                jsonify(
                    ok=True,
                    id=dup.id,
                    redirect=url_for("fraction.library_detail", template_id=dup.id),
                ),
                201,
            )
...
def _parse_severity(value: Any) -> Severity | None:
    ...
    Fraction's own ``fraction.enums.Severity`` -- docs/LOTEK_ADOPTION.md §3.2). The return type hint
