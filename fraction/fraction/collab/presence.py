...
last-writer-wins autosave (see ``fraction/autosave_api.py``) guarded only by this soft, advisory signal
...
# roughly half this interval (see fraction/static/editor.js PRESENCE_HEARTBEAT_MS).
...
# when re-exported from fraction/collab/__init__.py (`from fraction.collab.presence import presence`
# rebinds the package attribute `fraction.collab.presence` from "the submodule" to "this instance",
# breaking `import fraction.collab.presence as x`). Callers reach it as
# `fraction.collab.presence.registry` or via the submodule import.
