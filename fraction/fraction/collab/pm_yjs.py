...
Fraction's canonical content is ProseMirror JSON (``fraction/content/schema.py``, frozen). Phase B adds
...
Fraction currently ships only a documented client stub — see ``fraction/static/collab.js``).
...
def normalize_doc(pm_doc: dict | None) -> dict:
    ...
    (the "C2 freshness" check in ``fraction/collab/crdt.py``)."""
    scratch = Y.Doc()
