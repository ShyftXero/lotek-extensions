...
Where Phase A (``fraction/autosave_api.py`` + ``fraction/collab/presence.py``) is debounced
...
- :class:`Room` — one ``pycrdt.Doc`` + ``pycrdt.Awareness`` for a single ``(finding_id, block)``. It
  ...
  vendored client (``fraction/static/collab.js`` + ``fraction/static/lib/``) is wire-compatible with
...
- The ProseMirror JSON <-> Yjs mapping lives in ``fraction/collab/pm_yjs.py``.
...
def _artifact_url(artifact_id: int) -> str:
    ...
    ``fraction.autosave_api._artifact_url`` — WS5's real serve route may not be wired at render time)."""
    try:
...
# Module-level singleton — one process, one registry (mirrors fraction/collab/presence.py's `registry`).
manager = RoomManager()
...
def register(api_bp, bp) -> None:
    ...
    ``plans/CONTRACTS.md`` / ``fraction/__init__.py:_wire_feature_routes``, which the driver — not this
